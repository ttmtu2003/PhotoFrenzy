...
const SignupForm = ({ className }) => {
  ...
  const { signupUser, error, setError } = useSignUp()

  const history = useHistory()

  const handleSubmit = async (e) => {
    e.preventDefault()
    const response = await signupUser(fullName, username, password);
    if (response) {
      history.push('/');
    }
  ...
  return (
    <div className="mt-3 t-w-fit t-font-light">
      <Form onSubmit={handleSubmit} className="t-flex t-justify-between t-flex-col">
        ...
        <Button
          type="submit"
          className="hover:t-cursor-pointer hover:t-bg-[#519bc3] t-w-full t-py-[10px] t-px-[20px] t-my-[0.6em] t-bg-[#64BCED] t-text-white t-font-bold t-rounded-md t-border-none"
        >
          Signup
        </Button>
        <div className="t-my-[0.6em]">
          Already have an account?
          <Link to="/" className="t-text-blue-500 t-underline">
            {' '}
            Sign in instead
          </Link>
        </div>
